...
| 1.12 | 2021-10-22 | SG | Add `null`-argument tests and fix a few mistakes.
| 1.13 | 2021-10-22 | SG | Add a few more tests for better coverage.
...
| `addArea` | `addInvalidArea` | Asserts that inserting an area with `null` name results in `DataAccessException` (thrown by "NOT NULL" database constraint). | `null, 55.8013888888889, 13.3336111111111` | Pass
...
| `getAllAreas` | `getAllAreasNotEmpty` | Asserts that the retrieved list of areas contains elements (should be pre-populated from market research). | | Pass
| `addArea` | `addInvalidArea` | Asserts that we receive a `BadRequestException` when attempting to add a `null` area. | `null` | Pass
...
| `addDriverRoute()` | `addInvalidDriverRoute()` | Asserts that we receive `BadRequestException` when attempting to add a `null` driver route. | `null` | Pass |
...
| `addDriverRoute()` | `addDriverRouteWithInvalidAreas()` | Asserts adding a driver route with an invalid area throws `BadRequestException` (thrown by foreign key database constraint). | `{"vehicleId":testVehicle.getId(), "maxAvailableSeats":4, "startAreaId":testStartArea.getId(), "endAreaId":-1, "departAt":testDate, "arriveAt":testDate}` | Pass |
...
| `addPassengerRoute` | `addInvalidPassengerRoute` | Asserts that we receive `BadRequestException` when attempting to add a `null` passenger route. | `null` | Pass |
...
| `addPassengerRoute` | `addPassengerRouteWithInvalidAreas` | Asserts adding a passenger route with an invalid area throws `BadRequestException` (thrown by foreign key database constraint). |  `{"id":"-1", "userId":"4", "startAreaId":"testStartArea.getId()", "endAreaId":-1", "departAt":"testDate", "arriveAt":"testDate"}` | Pass |
...
| `addVehicle` | `addInvalidVehicle` | Asserts that we receive `DataAccessException` when inserting a vehicle with `null` license plate (thrown by "NOT NULL" database constraint). | `TEST.getId(), null, "Volvo", "red", "XC60"` | Pass

### 2.4.2 `VehicleResource.java`

| Methods | Test | Description | Json / Query Params | Result |
|:-|:-|:-|:-|:-
| `getUserVehicles` | `getUserVehiclesIsEmpty` | Logs in as the ADMIN user and asserts that they have no registered vehicles. | | Pass
...
| `addVehicle` | `addInvalidVehicle` | Asserts that we receive `BadRequestException` when attempting to add a `null` vehicle. | `null` | Pass
...
No new tests have been added to the base system, only modified to fit our changes. Does not include all tests in `UserResourceTest.java`.
...
| `getUser` | `getUserAsUser` | Logs in as the TEST user, then asserts that we receive a `ForbiddenException` when trying to get the ADMIN user. | `ADMIN.getId()` | Fail (Reason being that we changed this near the deadline to allow retrieving information about matched drivers and passengers. This is something we could improve.)
