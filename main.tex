...
\lhead{ Responsible: Alexander Ekman \& Linnea Johnsson \\ Date: \today}  \rhead{Document number: SRS \\ Version: 1.0}
...
This Product Requirements Document (PRD) details the requirements to develop a web-app based carpool service called ETSN05\_PG1 Carpool Webb-app (\nameNospace). This introductory section gives an overview of the PRD including it's purpose, goals, system users, terminology, and reference documents.

The purpose of this PRD is to provide the system characteristics, the epics and user stories, as well as the functional and non-functional requirements of the system. This document is intended as a basis for the contract between the development team and the client, it will also serve as a reference for the development team during the development process.
...
The system user is either non-car-owner (rider) who would like to carpool somewhere, a car owner (driver) who is willing to bring someone along a planned route, or someone who sometimes is a driver and sometimes a rider.
...
Driver = A user who submitted that they will drive a certain route\newline
Rider = A user who submitted that they want to ride along with a driver on a certain route\newline
...
\begin{figure}[!htpb]
    ...
    \caption{A use case diagram of the \namep}
...
\paragraph{Trip matching algorithm}\label{req:tripDatabase}
\textbf{ID}: D5\newline
\textbf{Priority}: HIGH\newline
...
\textbf{Rationale}: In order for new members to use the service\newline
\textbf{Dependency}: NA
...
\paragraph{User log-out}\label{req:log-in}
\textbf{ID}: F4\newline
\textbf{Priority}: HIGH\newline
...
\paragraph{Necessary contact details}\label{req:log-in}
\textbf{ID}: F10\newline
\textbf{Priority}: HIGH\newline
...
\paragraph{Upcoming travel}\label{req:log-in}
\textbf{ID}: F11\newline
\textbf{Priority}: MEDIUM\newline
...
\paragraph{Admin log in}\label{req:log-in}
\textbf{ID}: F13\newline
\textbf{Priority}: HIGH\newline
...
\paragraph{Removing users}\label{req:log-in}
\textbf{ID}: F14\newline
\textbf{Priority}: HIGH\newline
...
\begin{figure}[!htpb]
    ...
    \begin{minipage}{0.25\textwidth}
        ...
        \caption{Screen shown to unauthenticated user. The window prompts for registered users, and guides unregistered users to register}
        \label{fig:login}
    \end{minipage}\hfill
    \begin{minipage}{0.25\textwidth}
        \centering
    ...
    \begin{minipage}{0.25\textwidth}
        ...
        \caption{Screen shown after successful log-in. Here riders can submit ride requests, and drivers submit drive instances}
...
\begin{figure}[!htpb]
    ...
    \begin{minipage}{0.25\textwidth}
        ...
        \caption{Profile screen showing user their current details and which are editable. It also shows upcoming upcoming trips and the chance to cancel them}
        \label{fig:profile}
    \end{minipage}\hfill
    \begin{minipage}{0.25\textwidth}
        \centering
